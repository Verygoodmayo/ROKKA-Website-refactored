...
export default function CoreTechnologySubMenu({state, changeState}: {state: boolean, changeState: (state: boolean) => void}) {
    return (
        <div className={`core-technology-sub-menu ${state ? 'open' : 'closed'}`}>
            <ROKKAFrame content={
                <>
                    {CoreTechnologySubMenuItems.map((item) => (
                        <MenuItem key={item.label} {...item} itemClass={`core-technology-sub-menu-item ${state ? 'open' : 'closed'} ${item.label}`} hasSubMenu={item.hasSubMenu} subMenuChangeState={() => item.hasSubMenu ? changeState(false) : null}/>
                    ))}
                </>
